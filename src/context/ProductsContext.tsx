...
export const ProductsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        const data = await fetchSupabase<Product[]>('/products?select=*&order=id.asc');
        setProducts(data ?? []);
      } catch (error) {
  ...
  const addProduct = async (productData: Omit<Product, 'id'>) => {
    if (isSupabaseEnabled) {
      try {
        ...
        console.error('Ошибка добавления товара в Supabase:', (error as Error).message);
      }
      return;
    }
  ...
  const deleteProduct = async (id: number) => {
    if (isSupabaseEnabled) {
      try {
        ...
        console.error('Ошибка удаления товара в Supabase:', (error as Error).message);
        return;
      }
    }
