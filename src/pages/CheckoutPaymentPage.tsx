      ...
      <main className="flex-grow w-full min-h-screen pt-24 pb-32 px-4 flex items-center justify-center">
        <div className="text-center max-w-lg">
          ...
          <p className="text-brown-light mb-8">
            ...
            <span className="text-salmon">{shippingData?.email}</span>
          </p>
          <Link
            to="/shop"
            className="inline-block bg-sand hover:bg-salmon text-white px-8 py-3 rounded-xl font-bold transition-all duration-300"
          >
            Continue Shopping
          </Link>
        </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-500 font-medium hidden sm:block">
                Shipping
              </span>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-6">
            <div className="bg-moccasin/50 backdrop-blur-sm rounded-2xl p-6 border border-sand/50">
              <h2 className="text-xl font-bold text-brown-dark mb-6">
                Payment Method
              </h2>
              <div className="grid sm:grid-cols-2 gap-3 mb-6">
                <label
                  className={`flex items-center gap-3 p-4 rounded-xl cursor-pointer transition-all ${
                    paymentMethod === 'card'
                      ? 'bg-sand/20 border-2 border-salmon'
                      : 'bg-moccasin/80 border-2 border-sand/50'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'card' ? 'border-salmon' : 'border-brown-light/50'}`}
                  >
                    <div
                      className={`w-3 h-3 rounded-full ${paymentMethod === 'card' && 'bg-salmon'}`}
                    />
                  </div>
                ...
                >
                  ...
                  />
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'paypal' ? 'border-salmon' : 'border-brown-light/50'}`}
                  >
                    <div
                      className={`w-3 h-3 rounded-full ${paymentMethod === 'paypal' && 'bg-salmon'}`}
                    />
                  </div>
            ...
            <div className="bg-moccasin/50 backdrop-blur-sm rounded-2xl p-6 border border-sand/50">
              <div className="flex justify-between items-center mb-4">
                ...
                <Link
                  ...
                  className="text-salmon hover:opacity-80 text-sm"
                >
                  Edit
                </Link>
              </div>
            ...
            <div className="flex gap-4">
              <Link
                to="/checkout/shipping"
                className="flex-1 bg-moccasin hover:bg-sand/50 text-brown-dark py-4 px-6 rounded-xl font-bold text-center transition-colors"
              >
                ← Back
              </Link>
              <button
                ...
                className={`flex-1 py-4 px-6 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2 ${
                  isProcessing
                    ? 'bg-sand/50 text-white cursor-not-allowed'
                    : 'bg-sand hover:bg-salmon text-white shadow-lg shadow-sand/40'
                }`}
              >
                {isProcessing ? (
                  <>
            ...
            <div className="bg-moccasin/50 backdrop-blur-sm rounded-2xl p-6 border border-sand/50 sticky top-24">
                ...
                {items.map(({ product, quantity }) => (
                  <div key={product.id} className="flex gap-3">
                    <img
                      ...
                      alt={product.name}
                      className="w-16 h-16 object-cover rounded-lg bg-cream-bg"
                    />
                    <div className="flex-grow">
                      <p className="text-brown-dark text-sm font-medium line-clamp-1">
                        {product.name}
                      </p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-brown-light">
                  ...
                  <span
                    className={
                      shippingCost === 0 ? 'text-green-600' : 'text-brown-dark'
                    }
                  >
                    {shippingCost === 0
                      ? 'FREE'
